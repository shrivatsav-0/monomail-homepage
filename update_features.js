...
const filePath = 'C:\\Users\\Shiro\\Projects\\Monomail\\index.html';
let content = fs.readFileSync(filePath, 'utf8');

// 1. Update CSS for features-grid
const oldCss = `            .features-grid {
                display: grid;
...
const oldMq = `                .features-grid {
                    grid-template-columns: 1fr;
                }`;
const newMq = `                .features-grid {
                    grid-template-columns: 1fr;
                }
                .feat-span-2 {
                    grid-column: span 1;
...
// replace the last occurrence (the one under 768px)
const index = content.lastIndexOf(oldMq);
if (index !== -1) {
    content = content.substring(0, index) + newMq + content.substring(index + oldMq.length);
}

// 3. Update the HTML
const newHtml = `                <div class="features-grid">
                    <!-- Row 1 -->
...
const pattern = /<div class="features-grid">[\s\S]*?<\/div>\s*<\/div>\s*<\/section>/;
